...
func NewModule(L *lua.LState, funs map[string]interface{}) int {
	mod := L.SetFuncs(L.NewTable(), func(ls *lua.LState, fs map[string]interface{}) map[string]lua.LGFunction {
		exports := make(map[string]lua.LGFunction)
		for i, value := range fs {
			if value == nil {
				continue
			}
			if _, ok := value.(lua.LValue); ok {
				continue
			}
			val := reflect.ValueOf(value)
			if val.Kind() == reflect.Func {
				exports[i] = funcWrapper(ls, val).GFunction
			}
		}
		return exports
	}(L, funs))
